...
#if WASM_ENABLE_BRANCH_HINTS != 0
static bool
handle_branch_hint_section(const uint8 *buf, const uint8 *buf_end, WASMModule *module, char *error_buf,
                    uint32 error_buf_size)
{
    if (module->function_hints == NULL) {
        module->function_hints = loader_malloc(sizeof(struct WASMCompilationHint)* module->function_count, error_buf, error_buf_size);
    }
    ...
    for (uint32 i = 0; i < numFunctionHints; ++i) {
        ...
        if (!check_function_index(module, func_idx, error_buf, error_buf_size)) {
            goto fail;
        }
        if (func_idx < module->import_function_count) {
            set_error_buf(error_buf, error_buf_size, "branch hint for imported function is not allowed");
            goto fail;
        }

        struct WASMCompilationHintBranchHint* current_hint = (struct WASMCompilationHintBranchHint*)&module->function_hints[func_idx - module->import_function_count];
        ...
        for (uint32 j = 0; j < num_hints; ++j) {
            struct WASMCompilationHintBranchHint* new_hint = loader_malloc(sizeof(struct WASMCompilationHintBranchHint), error_buf, error_buf_size);
            new_hint->next = NULL;
            ...
            if (size != 1) {
                set_error_buf_v(error_buf, error_buf_size, "invalid branch hint size, expected 1, got %d.", size);
            ...
            if (data == 0x00)
                ...
                set_error_buf_v(error_buf, error_buf_size, "invalid branch hint, expected 0 or 1, got %d", data);
                wasm_runtime_free(new_hint);
                goto fail;
            }
    ...
    if (buf != buf_end) {
        set_error_buf(error_buf, error_buf_size, "invalid branch hint section, not filled until end");
        goto fail;
    }
    return true;
fail:
                  ...
                  uint32 error_buf_size)
{
    ...
    if (name_len == 25 && memcmp(p, "metadata.code.branch_hint", 25) == 0) {
        p += name_len;
        if (!handle_branch_hint_section(p, p_end, module, error_buf, error_buf_size)) {
            return false;
        }
