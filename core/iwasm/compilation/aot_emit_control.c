...
static void aot_emit_branch_hint(LLVMContextRef ctxt, AOTFuncContext *func_ctx, uint32 offset, LLVMValueRef br_if_instr)
{
    struct WASMCompilationHintBranchHint *hint =
        (struct WASMCompilationHintBranchHint *)func_ctx->function_hints;
    while (hint != NULL) {
        if (hint->type == WASM_COMPILATION_BRANCH_HINT && hint->offset == offset) {
            break;
        }
        hint = hint->next;
    }
    if (hint != NULL) {
        ...
        const uint32_t max_weight = 1u << 31;
        LLVMMetadataRef true_w = LLVMValueAsMetadata(
            LLVMConstInt(LLVMInt32TypeInContext(ctxt),
                         hint->is_likely ? 0 : max_weight, false));
        LLVMMetadataRef false_w = LLVMValueAsMetadata(
            LLVMConstInt(LLVMInt32TypeInContext(ctxt),
                         hint->is_likely ? max_weight : 0, false));
    ...
    if (label_type == LABEL_TYPE_BLOCK || label_type == LABEL_TYPE_LOOP) {
        ...
        if (!LLVMIsEfficientConstInt(value)) {
            ...
            if (else_addr) {
                ...
                const uint32 off = *p_frame_ip - func_ctx->aot_func->code_body_begin;
